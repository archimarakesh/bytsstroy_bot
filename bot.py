...
async def send_lead(message: Message, state: FSMContext, consult: bool):
    ...
    if consult:
        ...
        note = (
            "Заявка из Telegram-бота — КОНСУЛЬТАЦИЯ\n"
            f"Имя: {name}\n"
            f"Телефон: {phone}\n\n"
        ...
        note = (
            "Заявка из Telegram-бота — КОНФИГУРАЦИЯ\n"
            f"Имя: {name}\n"
            f"Телефон: {phone}\n\n"
    ...
    ok = await create_amo_lead(title=title, name=name, phone=phone, note=note)
...
async def create_amo_lead(title: str, name: str, phone: str, note: str) -> bool:
    """
    Создаёт сделку в amoCRM через API v4 (метод «сложное добавление»):
    сделка + контакт с телефоном одним запросом, затем примечание с конфигурацией.
        ...
        async with aiohttp.ClientSession() as sess:
            ...
            async with sess.post(f"{AMO_BASE}/leads/complex",
                ...
                if isinstance(res, list) and res:
                    lead_id = res[0].get("id")

            # 2) добавляем примечание с конфигурацией к сделке
            if lead_id:
                note_payload = [{
                    "entity_id": lead_id,
                    "note_type": "common",
                ...
                async with sess.post(f"{AMO_BASE}/leads/{lead_id}/notes",
                                     headers=headers, json=note_payload, timeout=30) as rn:
                    if rn.status not in (200, 201):
                        ntext = await rn.text()
                        logging.warning("amoCRM note %s: %s", rn.status, ntext[:300])
            return True
    except Exception as e:
...
def await_json(text: str):
    """Безопасный разбор JSON-ответа."""
    import json
    try:
